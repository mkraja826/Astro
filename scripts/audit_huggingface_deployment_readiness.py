...
def read_text(path: Path) -> str:
    try:
        ...
        fail(f"Expected UTF-8 text file could not be decoded: {path.relative_to(ROOT)} ({exc})")
...
def main() -> int:
    ...
    if status:
        fail("The Astro working tree is not clean. Commit or stash local changes before auditing.")
    ...
    require_contains(dockerfile, "FROM python:3.12-slim", "Production Docker base image changed unexpectedly.")
    require_contains(dockerfile, "PORT=8080", "Production Dockerfile no longer defaults to port 8080.")
    require_contains(dockerfile, "EXPOSE 8080", "Production Dockerfile does not expose port 8080.")
    require_contains(dockerfile, EXPECTED_JPL_SHA256, "Pinned JPL DE440s checksum is missing from the Dockerfile.")
    require_contains(dockerfile, "USER api", "Production container must run as a non-root user.")
    require_contains(
        dockerfile,
    ...
    require_contains(manifest, "sdk: docker", "Hugging Face manifest must select the Docker SDK.")
    require_contains(manifest, "app_port: 8080", "Hugging Face manifest must expose port 8080.")
    require_contains(manifest, "JYOTHISYAM_API_KEY", "Space secret-name documentation is incomplete.")
    require_contains(
        manifest,
    ...
    for value in required_runbook_values:
        require_contains(runbook, value, f"Deployment runbook is missing required value: {value}")
    ...
    for marker in forbidden_value_markers:
        if marker in deployment_sources:
            fail("A credential-like value marker exists in Hugging Face deployment material.")
